...
async function fetchRepoStats() {
    try {
        ...
        if (stored) {
            ...
            const starCount = document.getElementById('starCount');
            const forkCount = document.getElementById('forkCount');
            if (starCount) starCount.innerHTML = `${formatNumber(stats.stars || 0)}`;
            if (forkCount) forkCount.innerHTML = `${formatNumber(stats.forks || 0)}`;
        }
        return null;
    }
}

// --- Celebrate Function for Star/Fork ---
...
function initPromoButtons() {
    const starBtn = document.getElementById('starPromoBtn');
    const forkBtn = document.getElementById('forkPromoBtn');
    
    if (starBtn) {
        starBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            celebrateAction('star');
        });
    }
...
// Call initApp instead of directly loading
initApp();
    ...
    // Get pinned repos (top 3 by stars)
...
<div align="center">
  ...
  [![GitHub Sponsors](https://img.shields.io/badge/Sponsor-GitHub%20Sponsors-pink?style=social&logo=githubsponsors)](https://github.com/sponsors/${user.login})
</div>
...
${topLangs.map(([lang, count]) => `- **${lang}:** ${count} repositories`).join('\n')}
...
[![GitHub Sponsors](https://img.shields.io/badge/Sponsor-@${user.login}-ea4aaa?style=for-the-badge&logo=githubsponsors)](https://github.com/sponsors/${user.login})
[![Buy Me A Coffee](https://img.shields.io/badge/Buy%20Me%20A%20Coffee-@${user.login}-FFDD00?style=for-the-badge&logo=buymeacoffee)](https://www.buymeacoffee.com/${user.login})
[![Ko-fi](https://img.shields.io/badge/Ko--fi-@${user.login}-FF5E5B?style=for-the-badge&logo=kofi)](https://ko-fi.com/${user.login})
